...
async function fetchWords() {
  ...
  const scoreElem = document.getElementById('score').innerHTML = "0/" + totalWords;
...
function pickWordAndInitialize() {
  ...
  const wordObject = wordsList.splice(Math.floor(Math.random() * wordsList.length), 1)[0];
  wordToGuess = wordObject.word;
  let effectiveLength = wordToGuess.replace(/-/g, "").length;
  damageToEnemy = Math.ceil(100 / Math.max(effectiveLength - 1, 1));
  console.log("damage to enemy : " + damageToEnemy);
  displayedWord = wordToGuess.split('').map(char => char === '-' ? '-' : '_');
  revealLetter(wordToGuess[0]);
  document.getElementById('wordToGuess').innerText = displayedWord.join(' ');
  document.getElementById('status').innerText = '';
  document.getElementById('hint').innerText = wordObject.hint; // Display the hint
